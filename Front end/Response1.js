...
const Response = ({ route }) => {
  ...
  const questionContents = [
    ...
    "Peritoneal Free Fluid / Mesentric Fat Stranding"
  ];

  useEffect(() => {
    fetch(`${API_BASE_URL}/getresponse1.php?userId=${userId}&date=${date}`)
  ...
  const renderContentForQuestion = (index, response) => {
    if (index === 0 && response.answer === 'Option 1') {
      return <Text>More than 5</Text>;
    } else if (index === 0 && response.answer === 'Option 2') {
      return <Text>Less than 5</Text>;
    }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.managementContainer}>
        <Text style={[styles.managementText, { color: totalScore <= 20 ? 'green' : 'red' }]}>
          {management}
        </Text>
      </View>
